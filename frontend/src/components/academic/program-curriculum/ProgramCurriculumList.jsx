...
import {
    ...
    faEllipsisVertical, faRefresh, faSearch, faTimes, faGraduationCap,
    faBook, faListUl
...
const ProgramCurriculumList = () => {
    ...
    const loadProgramCurriculums = async (page = 0, size = pageSize, search = '', searchBy = 'majorName') => {
        try {
            setLoading(true);
            setError(null);
            ...
            // Handle response structure with placeholder data for development
            let programCurriculumList = [];
            
            if (response.result) {
                programCurriculumList = response.result || [];
            } else {
                // 🚧 PLACEHOLDER DATA - Replace when backend is ready
                console.warn('🚧 Using placeholder program curriculum data - implement backend endpoint');
            ...
            if (search.trim()) {
                programCurriculumList = programCurriculumList.filter(curriculum => {
                    const searchValue = search.toLowerCase();
                    switch (searchBy) {
                        case 'majorName':
                            return curriculum.majorName?.toLowerCase().includes(searchValue);
                        case 'id':
            ...
            const paginatedCurriculums = programCurriculumList.slice(startIndex, endIndex);
            
            setProgramCurriculums(paginatedCurriculums);
            setTotalPages(Math.ceil(programCurriculumList.length / size));
            setTotalElements(programCurriculumList.length);
            setCurrentPage(page);
    ...
    const handleDeleteProgramCurriculum = async (curriculum) => {
        if (window.confirm(`Bạn có chắc chắn muốn xóa chương trình đào tạo cho ngành "${curriculum.majorName}"?`)) {
            try {
    ...
    const getSearchTypeDisplayText = (type) => {
        switch (type) {
            case 'majorName': return 'Tên Ngành';
            case 'id': return 'ID';
            default: return type;
        }
    };
        ...
        <MainLayout activeMenu="curriculum">
            <div className="container-fluid pt-3 pb-5">
                ...
                <div className="d-flex justify-content-between align-items-center mb-4">
                    <div>
                        <h2 className="h4 fw-bold text-dark">Quản Lý Chương Trình Đào Tạo</h2>
                    ...
                    <div className="d-flex gap-2">
                        <Button
                            ...
                            onClick={() => loadProgramCurriculums(currentPage, pageSize, searchTerm, searchType)}
                            disabled={loading}
                        >
                    ...
                    <div className="card-body p-0">
                            ...
                            <div className="d-flex align-items-center gap-3">
                                <div className="d-flex align-items-center">
                                    ...
                                    <Dropdown>
                                        ...
                                        <Dropdown.Menu>
                                            <Dropdown.Item onClick={() => handleSearchTypeChange('majorName')}>
                                                Tên Ngành
                                            </Dropdown.Item>
                        ...
                        {loading && (
                            <div className="text-center py-5">
                                ...
                                <p className="mt-2 text-muted">Đang tải danh sách chương trình đào tạo...</p>
                            </div>
                        )}
                                    ...
                                    <Table hover className="mb-0">
                                        <thead className="bg-light">
                                            <tr>
                                                ...
                                                <th scope="col" className="fw-medium text-muted">ID</th>
                                                <th scope="col" className="fw-medium text-muted">Tên Ngành</th>
                                                <th scope="col" className="fw-medium text-muted text-center">Số Môn Học</th>
                                                <th scope="col" className="fw-medium text-muted text-center">Tổng Tín Chỉ</th>
                                            ...
                                            {programCurriculums.length > 0 ? (
                                                    ...
                                                    <tr key={curriculum.id}>
                                                        ...
                                                        <td className="text-center">
                                                            <Dropdown>
                                                                ...
                                                                <Dropdown.Menu>
                                                                    <Dropdown.Item onClick={() => handleViewProgramCurriculum(curriculum)}>
                                                                        <FontAwesomeIcon icon={faEye} className="me-2" />
                                                                        Xem Chi Tiết
                                                                    </Dropdown.Item>
                                                                    <Dropdown.Item onClick={() => handleEditProgramCurriculum(curriculum)}>
                                                                    ...
                                                                    <Dropdown.Item 
                                                                        ...
                                                                        onClick={() => handleDeleteProgramCurriculum(curriculum)}
                                                                    >
                                                ...
                                                <tr>
                                                    <td colSpan="7" className="text-center py-5 text-muted">
                                                        <FontAwesomeIcon icon={faListUl} size="3x" className="mb-3 opacity-50" />
                                                        <p>Không có chương trình đào tạo nào được tìm thấy</p>
                                                        {searchTerm && (
                                                            <Button 
                                ...
                                {totalPages > 1 && (
                                    <div className="d-flex justify-content-between align-items-center p-3 border-top">
                                        <div className="text-muted small">
                                            Hiển thị {currentPage * pageSize + 1} - {Math.min((currentPage + 1) * pageSize, totalElements)} của {totalElements} chương trình đào tạo
                                        </div>
